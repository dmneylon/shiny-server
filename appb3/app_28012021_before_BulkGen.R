      ...
      tabItem(tabName = "proposedbuild",
              mainPanel(
                wellPanel(h4(tags$strong("Proposed Build:")), tags$hr(),
                          ...
                          selectInput("BuildType", "Build Type:", choices = c("New Build", "B1 - B3 Upgrade", "B2 - B3 Upgrade", "Tech Add only", "Preparatory", "Transition", ""), selected = ""),
...
server <- function(input, output, session) { 
  ...
  v6B <- reactive({
    ...
    ProdInv$Qty[113] <- sum(ifelse(substr(v6A(),23,23) == 6, 1, 0), ifelse(substr(v6A(),48,48) == "E", 1, 0), ifelse(substr(v6A(),nchar(v6A())-1,nchar(v6A())-1) == 5, 1, 0))
