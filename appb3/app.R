...
bulkgen <- as.data.frame(read.csv("QcodeTranslatorBulk.csv"))
...
ui <- dashboardPage(
  ...
  dashboardSidebar(width = 300,
    sidebarMenu(
      ...
      menuItem("Bulk Generator Report", tabName = "bulkgen", icon = icon("file-pdf")),
  ...
  dashboardBody(
    ...
    tabItems(
      ...
      # 24th tab content
      tabItem(tabName = "bulkgen",
              mainPanel(
                DT::dataTableOutput("bulkgen", width = "100%", height = "auto"),
...
# variable to identify SCD #    
  ...
  STR <- reactive({
    ...
    else if(vSCD()=="CCR")     # adapt this section of the else if statement to cover CCRs and one-offs
    {
      paste("T")
    }
    else if(str_detect(input$Config,"Stealth"))
    {
      paste("M")                   # For now we will use "M" for Stealth. Had been using "S" but causes problems for other radios that don't have a "S" assigned to them. There are only two "S" radios
    }                              # in the Product Inventory, Radio 2 (3TRX SM instead of the 4TRX SM for the "M" type counterpart) and Radio 6 (6TRX SM instead of the 8TRX SM for the "M" type counterpart).
    else                           # Need to figure out how we can introduce the functionality to have a mix of "M" and "S" specific radios in 1 configuration or if it is even necessary at all considering the negligible differences in Power Consumption data.
    {
      paste("M")
    }
  })
...
## Rendering Q-codes ##
 ...
 # Column Header Indexes #
  ...
  observe({print(paste("Testing v6A1: ",v6A1()))})
...
## Mappings section ##  
  ...
  observe({print(paste("Testing Cabinet 1 is: ",CAB1()))})
  ...
  vColCab1 <- reactive({
    ...
    else if(nchar(toString(v6A2())) < 51 | DS6456[which(DS6456$TelefonicaConfiguration == input$Config),159]==0 | DS6456[which(DS6456$TelefonicaConfiguration == input$Config),160]==0) # columns FC & FD
       {
         return (158)
    }
    else if(nchar(toString(v6A2())) > 50 & substr(v6A2(),50,54)=="56E21")
    {
      return (160)
    }
    else if(nchar(toString(v6A2())) > 50 & substr(v6A2(),50,53)=="56E7")
    {
      return (160)
    }
    else if(nchar(toString(v6A2())) > 50 & substr(v6A2(),50,54)=="56E34")
    {
      return (159)
    }
  })
  ...
  v1A <- reactive({
    if(CAB1() == Cab1NB)
    {
      paste(substr(CAB1(), 5, nchar(toString(CAB1()))-4),DS6456[which(DS6456$TelefonicaConfiguration == input$Config),vColCab1()],".pdf",sep="")
    }
    else
    {
      if(input$Config == "")
      ...
      else if(DS6456[which(DS6456$TelefonicaConfiguration == input$Config),vColCab1()]==0)
      {
        paste(substring(BLANK, 5),sep="")
      }
      else if(nchar(toString(DS6456[which(DS6456$TelefonicaConfiguration == input$Config),vColCab1()])) == 1)
      {
        paste(substr(CAB1(), 5, nchar(toString(CAB1()))-4),"0",DS6456[which(DS6456$TelefonicaConfiguration == input$Config),vColCab1()],".pdf",sep="")
      }
  ...
  vColCab2 <- reactive({
    ...
    else if(nchar(toString(v6A2())) < 51 | DS6456[which(DS6456$TelefonicaConfiguration == input$Config),162]==0 | DS6456[which(DS6456$TelefonicaConfiguration == input$Config),163]==0)  # columns FF & FG
    {
      return (161)
    }
    else if(nchar(toString(v6A2())) > 50 & substr(v6A2(),50,54)=="56E21")
  ...
  v1B <- reactive({
    ...
    else if(DS6456[which(DS6456$TelefonicaConfiguration == input$Config),vColCab2()]==0 | CAB1() == Cab1NB)
    {
      paste(substring(BLANK, 5),sep="")
    }
    else if(nchar(toString(DS6456[which(DS6456$TelefonicaConfiguration == input$Config),vColCab2()])) == 1)
    {
      paste(substr(CAB2(), 5, nchar(toString(CAB2()))-4),"0",DS6456[which(DS6456$TelefonicaConfiguration == input$Config),vColCab2()],".pdf",sep="")
    }
    else if(nchar(toString(DS6456[which(DS6456$TelefonicaConfiguration == input$Config),vColCab2()])) == 2)
    {
      paste(substr(CAB2(), 5, nchar(toString(CAB2()))-4),"",DS6456[which(DS6456$TelefonicaConfiguration == input$Config),vColCab2()],".pdf",sep="")
    }
    else
    {
      paste(substr(CAB2(), 5, nchar(toString(CAB2()))-4),DS6456[which(DS6456$TelefonicaConfiguration == input$Config),vColCab2()],".pdf",sep="")  
  ...
  #     {
  #       paste(substr(CAB, 5, nchar(toString(CAB))-4),"0",DS6456[which(DS6456$TelefonicaConfiguration == input$Config),35],".pdf",sep="")
  #     }
  #     else if(nchar(toString(DS6456[which(DS6456$TelefonicaConfiguration == input$Config),35])) == 2)
  #     {
  #       paste(substr(CAB, 5, nchar(toString(CAB))-4),"",DS6456[which(DS6456$TelefonicaConfiguration == input$Config),35],".pdf",sep="")
  #     }
  #     else
  #     {
  #       paste(substr(CAB, 5, nchar(toString(CAB))-4),DS6456[which(DS6456$TelefonicaConfiguration == input$Config),35],".pdf",sep="")  
  ...
  v1D <- reactive({
    if(v1Dpre()==0 | input$Config == "")
    {
      paste(substring(BLANK, 5),sep="")
            ...
            {
      paste(substr(CD(), 5, nchar(toString(CD()))-4), "0", v1Dpre(),".pdf",sep="")
    }
    else if(v1Dpre()!=0 & nchar(toString(v1Dpre()))==2)
            {
      paste(substr(CD(), 5, nchar(toString(CD()))-4), "", v1Dpre(),".pdf",sep="")    
    }
  })
  ...
  v1E <- reactive({
    ...
    {
      paste(substr(CD(), 5, nchar(toString(CD()))-4), "0", v1Epre(),".pdf",sep="")
    }
    else if(v1Epre()!=0 & nchar(toString(v1Epre()))==2)
    {
      paste(substr(CD(), 5, nchar(toString(CD()))-4), "", v1Epre(),".pdf",sep="")    
    }
  })  
  ...
  v1Ea <- reactive({
    ...
    {
      paste(substr(CD(), 5, nchar(toString(CD()))-4), "0", v1Eapre(),".pdf",sep="")
    }
    else if(v1Eapre()!=0 & nchar(toString(v1Eapre()))==2)
    {
      paste(substr(CD(), 5, nchar(toString(CD()))-4), "", v1Eapre(),".pdf",sep="")    
    }
  })  
  ...
  v1Eb <- reactive({
    ...
    {
      paste(substr(CD(), 5, nchar(toString(CD()))-4), "0", v1Ebpre(),".pdf",sep="")
    }
    else if(v1Ebpre()!=0 & nchar(toString(v1Ebpre()))==2)
    {
      paste(substr(CD(), 5, nchar(toString(CD()))-4), "", v1Ebpre(),".pdf",sep="")    
    }
  })  
  ...
  v1Ec <- reactive({
    ...
    {
      paste(substr(CD(), 5, nchar(toString(CD()))-4), "0", v1Ecpre(),".pdf",sep="")
    }
    else if(v1Ecpre()!=0 & nchar(toString(v1Ecpre()))==2)
    {
      paste(substr(CD(), 5, nchar(toString(CD()))-4), "", v1Ecpre(),".pdf",sep="")    
    }
  })    
  ...
  v1F <- reactive({
    ...
    {
      paste(substr(CPRI(), 5, nchar(toString(CPRI()))-4),"41",".pdf",sep="")
    }
    else if(CPRI()==CPRIB2B3 & v1Dpre()==60)
    {
      paste(substr(CPRI(), 5, nchar(toString(CPRI()))-4),"59",".pdf",sep="")
    }
    else if(v1Dpre()!=0 & nchar(toString(v1Dpre()))==1)
    {
      paste(substr(CPRI(), 5, nchar(toString(CPRI()))-4),"0",v1Dpre(),".pdf",sep="")
    }
    else if(v1Dpre()!=0 & nchar(toString(v1Dpre()))==2)
    {
      paste(substr(CPRI(), 5, nchar(toString(CPRI()))-4),v1Dpre(),".pdf",sep="")
    }
  })
  observe({print(v1F())})
  ...
  v1G <- reactive({
    ...
    {
      paste(substr(CPRI(), 5, nchar(toString(CPRI()))-4),"0",v1Epre(),".pdf",sep="")
    }
    else if(v1Epre()!=0 & nchar(toString(v1Epre()))==2)
    {
      paste(substr(CPRI(), 5, nchar(toString(CPRI()))-4),v1Epre(),".pdf",sep="")
    }
  })
  observe({print(v1G())})
  ...
  v1Ga <- reactive({
    ...
    {
      paste(substr(CPRI(), 5, nchar(toString(CPRI()))-4),"0",v1Eapre(),".pdf",sep="")
    }
    else if(v1Eapre()!=0 & nchar(toString(v1Eapre()))==2)
    {
      paste(substr(CPRI(), 5, nchar(toString(CPRI()))-4),v1Eapre(),".pdf",sep="")
    }
  })
  observe({print(v1Ga())})
  ...
  v1Gb <- reactive({
    ...
    {
      paste(substr(CPRI(), 5, nchar(toString(CPRI()))-4),"0",v1Ebpre(),".pdf",sep="")
    }
    else if(v1Ebpre()!=0 & nchar(toString(v1Ebpre()))==2)
    {
      paste(substr(CPRI(), 5, nchar(toString(CPRI()))-4),v1Ebpre(),".pdf",sep="")
    }
  })
  observe({print(v1Gb())})
...
# When match found, calculate quantity, Radios rows 2-99, DU/BB 100-124, Cabinets 125-136, remember starts at ProdInv$Qty[1] as this is the first row, header row not counted #  
    v6B <- reactive({
      ...
      for (looper in 1:numloops)       # the loop is used in bulk generation mode, it will be bypassed in the if/else statement if the bulk generation BuilType/Mode is not selected
      {
        
        if(input$BuildType == "Bulk Generator")
        {
          bulkvar <- bulkgen$ConcatenatedQCx3[looper]
        }
        else
        {
          bulkvar <- v6A()
        ...
        # Radios #
        ProdInv$Match <- ifelse(str_detect(bulkvar, regex(ProdInv$QcodePattern, ignore.case = T)), 1, 0)
        # GSM 900 SM Radios #
        ProdInv$Qty[1]<- ProdInv$Match[1]* as.numeric(substr(bulkvar,5,5))
        ProdInv$Qty[2]<- ProdInv$Match[2]* as.numeric(substr(bulkvar,5,5))
        ProdInv$Qty[3]<- ProdInv$Match[3]* as.numeric(substr(bulkvar,5,5))
        ProdInv$Qty[4]<- ProdInv$Match[4]* as.numeric(substr(bulkvar,5,5))
        ProdInv$Qty[5]<- ProdInv$Match[5]* as.numeric(substr(bulkvar,5,5))
        
        # UMTS SM Radios #
        ProdInv$Qty[6]<- ProdInv$Match[6]* as.numeric(substr(bulkvar,5,5))
        ProdInv$Qty[7]<- ProdInv$Match[7]* as.numeric(substr(bulkvar,19,19))
        ProdInv$Qty[8]<- ProdInv$Match[8]* as.numeric(substr(bulkvar,19,19))
        ProdInv$Qty[9]<- ProdInv$Match[9]* as.numeric(substr(bulkvar,19,19))
        ProdInv$Qty[10]<- ProdInv$Match[10]* as.numeric(substr(bulkvar,19,19))
        ProdInv$Qty[11]<- ProdInv$Match[11]* as.numeric(substr(bulkvar,19,19))
        ProdInv$Qty[12]<- ProdInv$Match[12]* as.numeric(substr(bulkvar,19,19))
        ProdInv$Qty[13]<- ProdInv$Match[13]* as.numeric(substr(bulkvar,17,17))
        ProdInv$Qty[14]<- ProdInv$Match[14]* as.numeric(substr(bulkvar,17,17))
        ProdInv$Qty[15]<- ProdInv$Match[15]* as.numeric(substr(bulkvar,17,17))
        ProdInv$Qty[16]<- ProdInv$Match[16]* as.numeric(substr(bulkvar,17,17))
        ProdInv$Qty[17]<- ProdInv$Match[17]* as.numeric(substr(bulkvar,19,19))
        ProdInv$Qty[18]<- ProdInv$Match[18]* as.numeric(substr(bulkvar,19,19))
        
        
        # LTE SM Radios #
        ProdInv$Qty[19]<- ProdInv$Match[19]* as.numeric(substr(bulkvar,32,32))
        ProdInv$Qty[20]<- ProdInv$Match[20]* as.numeric(substr(bulkvar,32,32))
        ProdInv$Qty[21]<- ifelse(as.numeric(substr(bulkvar,35,35) == "X"), 0, ProdInv$Match[21]* as.numeric(substr(bulkvar,35,35)))
        ProdInv$Qty[22]<- ifelse(as.numeric(substr(bulkvar,35,35) == "X"), 0, ProdInv$Match[22]* as.numeric(substr(bulkvar,35,35)))
        ProdInv$Qty[23]<- ProdInv$Match[23]* as.numeric(substr(bulkvar,41,41))
        ProdInv$Qty[24]<- ProdInv$Match[24]* as.numeric(substr(bulkvar,38,38))
        ProdInv$Qty[25]<- ProdInv$Match[25]* as.numeric(substr(bulkvar,38,38))
        ProdInv$Qty[26]<- ProdInv$Match[26]* as.numeric(substr(bulkvar,29,29))
        ProdInv$Qty[27]<- ProdInv$Match[27]* as.numeric(substr(bulkvar,29,29))
        ProdInv$Qty[28]<- ProdInv$Match[28]* as.numeric(substr(bulkvar,29,29))
        ProdInv$Qty[29]<- ifelse(as.numeric(substr(bulkvar,44,44) == "X"), 0, ProdInv$Match[29]* as.numeric(substr(bulkvar,44,44)))
        ProdInv$Qty[30]<- ifelse(as.numeric(substr(bulkvar,44,44) == "X"), 0, ProdInv$Match[30]* as.numeric(substr(bulkvar,44,44)))
        ProdInv$Qty[31]<- ifelse(as.numeric(substr(bulkvar,35,35) == "X"), 0, ProdInv$Match[31]* as.numeric(substr(bulkvar,35,35)))
        ProdInv$Qty[32]<- ProdInv$Match[32]* as.numeric(substr(bulkvar,32,32))
        ProdInv$Qty[33]<- ifelse(as.numeric(substr(bulkvar,44,44) == "X"), 0, ProdInv$Match[33]* as.numeric(substr(bulkvar,44,44)))
        ProdInv$Qty[34]<- ProdInv$Match[34]* as.numeric(substr(bulkvar,29,29))
        ProdInv$Qty[35]<- ifelse(as.numeric(substr(bulkvar,35,35) == "X"), 0, ProdInv$Match[35]* as.numeric(substr(bulkvar,35,35)))
        ProdInv$Qty[36]<- ProdInv$Match[36]* as.numeric(substr(bulkvar,32,32))
        ProdInv$Qty[37]<- ProdInv$Match[37]* as.numeric(substr(bulkvar,32,32))
        ProdInv$Qty[38]<- ProdInv$Match[38]* as.numeric(substr(bulkvar,32,32))
        ProdInv$Qty[39]<- ProdInv$Match[39]* as.numeric(substr(bulkvar,29,29))
        ProdInv$Qty[40]<- ProdInv$Match[40]* as.numeric(substr(bulkvar,32,32))
        ProdInv$Qty[41]<- ProdInv$Match[41]* as.numeric(substr(bulkvar,32,32))
        ProdInv$Qty[42]<- ProdInv$Match[42]* as.numeric(substr(bulkvar,32,32))
        ProdInv$Qty[43]<- ProdInv$Match[43]* as.numeric(substr(bulkvar,32,32))
        ProdInv$Qty[44]<- ProdInv$Match[44]* as.numeric(substr(bulkvar,29,29))
        ProdInv$Qty[45]<- ProdInv$Match[45]* as.numeric(substr(bulkvar,29,29))
        ProdInv$Qty[46]<- ProdInv$Match[46]* as.numeric(substr(bulkvar,32,32))
        ProdInv$Qty[47]<- ProdInv$Match[47]* as.numeric(substr(bulkvar,32,32))
        ProdInv$Qty[48]<- ifelse(as.numeric(substr(bulkvar,35,35) == "X"), 0, ProdInv$Match[48]* as.numeric(substr(bulkvar,35,35)))
        ProdInv$Qty[49]<- ifelse(as.numeric(substr(bulkvar,35,35) == "X"), 0, ProdInv$Match[49]* as.numeric(substr(bulkvar,35,35)))
        ProdInv$Qty[50]<- ProdInv$Match[50]* as.numeric(substr(bulkvar,38,38))
        ProdInv$Qty[51]<- ProdInv$Match[51]* as.numeric(substr(bulkvar,38,38))
        ProdInv$Qty[52]<- ifelse(as.numeric(substr(bulkvar,35,35) == "X"), 0, ProdInv$Match[52]* as.numeric(substr(bulkvar,35,35)))
        ProdInv$Qty[53]<- ProdInv$Match[53]* as.numeric(substr(bulkvar,32,32))
        ProdInv$Qty[54]<- ProdInv$Match[54]* as.numeric(substr(bulkvar,32,32))
        
        
        # GSM & UMTS MM Radios
        ProdInv$Qty[55]<- ProdInv$Match[55]* as.numeric(substr(bulkvar,5,5))
        ProdInv$Qty[56]<- ProdInv$Match[56]* as.numeric(substr(bulkvar,5,5))
        ProdInv$Qty[57]<- ProdInv$Match[57]* as.numeric(substr(bulkvar,5,5))
        ProdInv$Qty[58]<- ProdInv$Match[58]* as.numeric(substr(bulkvar,5,5))
        ProdInv$Qty[59]<- ProdInv$Match[59]* as.numeric(substr(bulkvar,5,5))
        ProdInv$Qty[60]<- ProdInv$Match[60]* as.numeric(substr(bulkvar,5,5))
        ProdInv$Qty[61]<- ProdInv$Match[61]* as.numeric(substr(bulkvar,5,5))
        ...
        # 5G NR SM Radios 
        ProdInv$Qty[82]<- ProdInv$Match[82]* 3
        ProdInv$Qty[83]<- ProdInv$Match[83]* 2
        ProdInv$Qty[84]<- ProdInv$Match[84]* 2
        ProdInv$Qty[85]<- ProdInv$Match[85]* 6
        ProdInv$Qty[86]<- ProdInv$Match[86]* 6
        ProdInv$Qty[87]<- ProdInv$Match[87]* 4
        ...
        # ProdInv$Qty[XX]<- ifelse(substr(bulkvar,32,32)== "A", ProdInv$Match[XX]*10, 0)
        # ProdInv$Qty[XX]<- ifelse(substr(bulkvar,32,32)== "B", ProdInv$Match[XX]*11,0)
        # ProdInv$Qty[XX]<- ifelse(substr(bulkvar,32,32)== "C", ProdInv$Match[XX]*12, 0)
        # ProdInv$Qty[XX]<- ifelse(substr(bulkvar,32,32)== "D", ProdInv$Match[XX]*13, 0)
        # ProdInv$Qty[XX]<- ifelse(substr(bulkvar,32,32)== "E", ProdInv$Match[XX]*14, 0)
        # ProdInv$Qty[XX]<- ifelse(substr(bulkvar,32,32)== "F", ProdInv$Match[XX]*15, 0)
        ...
        # DUG 20
        ProdInv$Qty[100] <- ifelse(substr(bulkvar,11,11) == 1, 3, ifelse(substr(bulkvar,11,11) == 2, 4, ifelse(substr(bulkvar,11,11) == 3, 6, ifelse(substr(bulkvar,11,11) == 4, 4,             
                                                                                                                                                     ifelse(substr(bulkvar,11,11) == 5, 2, ifelse(substr(bulkvar,11,11) == 6, 3, ifelse(substr(bulkvar,11,11) == 7, 2, ifelse(substr(bulkvar,11,11) == 8, 1,                                                                                                                   
                                                                                                                                                                                                                                                                              ifelse(substr(bulkvar,11,11) == 9, 1, ifelse(substr(bulkvar,11,11) == 9, 1,0))))))))))
        
        # DUW 20
        ProdInv$Qty[101] <- ifelse(substr(bulkvar,23,23) == 9, 1, 0)
        
        # DUW 30
        ProdInv$Qty[102] <- ifelse(substr(bulkvar,23,23) == "K" | substr(bulkvar,23,23) == "Q" | substr(bulkvar,23,23) == "S" | substr(bulkvar,23,23) == 9, 1,
                                   ifelse(substr(bulkvar,23,23) == "F" | substr(bulkvar,23,23) == "L" | substr(bulkvar,23,23) == "M" | substr(bulkvar,23,23) == "P" | substr(bulkvar,23,23) == "T", 2,
                                          ifelse(substr(bulkvar,23,23) == "N" | substr(bulkvar,23,23) == "4", 3,
                                                 ifelse(substr(bulkvar,23,23) == 5, 4, 0))))
        
        # DUW 31
        ProdInv$Qty[103] <- ifelse(substr(bulkvar,23,23) == "A" | substr(bulkvar,23,23) == "F" | substr(bulkvar,23,23) == "K" | substr(bulkvar,23,23) == "L" 
                                   | substr(bulkvar,23,23) == "M" | substr(bulkvar,23,23) == "Q" | substr(bulkvar,23,23) == 4, 1,
                                   ifelse(substr(bulkvar,23,23) == "B" | substr(bulkvar,23,23) == "C", 2,
                                          ifelse(substr(bulkvar,23,23) == "D", 3,
                                                 ifelse(substr(bulkvar,23,23) == "E", 4, 0))))
        
        # DUS 31
        ProdInv$Qty[104] <- ifelse(substr(bulkvar,49,49) == 1, 1, ifelse(substr(bulkvar,49,49) == 2, 2, ifelse(substr(bulkvar,49,49) == 3, 3, ifelse(substr(bulkvar,49,49) == 4, 4, 0))))
        
        # # Baseband 5212
        # ProdInv$Qty[105] <- ifelse(substr(bulkvar,48,48) == 5, 2, 0)
        
        # # Baseband 5216
        # ProdInv$Qty[106] <- ifelse(substr(bulkvar,23,23) == "H", 2, ifelse(substr(bulkvar,23,23) == "G" | substr(bulkvar,23,23) == "J", 1,
        #                    ifelse(substr(bulkvar,48,48) == 3 | substr(bulkvar,48,48) == 4 | substr(bulkvar,48,48) == 6, 1, 0)))
        
        # Baseband 6620 with fan (BB6620)
        ProdInv$Qty[107] <- sum(ifelse(substr(bulkvar,48,48) == 8, 2, ifelse(substr(bulkvar,48,48) == "B", 1, 0)),
                                ifelse(substr(bulkvar,23,23) == 7, 1,0))
        ...
        ProdInv$Qty[108] <- if(substr(bulkvar,nchar(toString(bulkvar))-1,nchar(toString(bulkvar))-1) == "*")     # that is no 5G
        {
          sum(ifelse(substr(bulkvar,11,11) == "A", 0, 0),   # count as 0 as thhis Mixed Mode Baseband will be counted in the 3G Q-code
              ...
              ifelse(substr(bulkvar,48,48) == "G" | substr(bulkvar,48,48) == "H",3,0))
        }
        else
        {
          sum(ifelse(substr(bulkvar,11,11) == "A", 0, 0),   # count as 0 as thhis Mixed Mode Baseband will be counted in the 3G Q-code
              ...
              ifelse(substr(bulkvar,48,48) == 7 | substr(bulkvar,48,48) == "A", 2, 0),
              ifelse(substr(bulkvar,48,48) == "G" | substr(bulkvar,48,48) == "H",3,0),
              ifelse(substr(bulkvar,nchar(toString(bulkvar))-1,nchar(toString(bulkvar))-1) == 1 | substr(bulkvar,nchar(toString(bulkvar))-1,nchar(toString(bulkvar))-1) == "C" | substr(bulkvar,nchar(toString(bulkvar))-1,nchar(toString(bulkvar))-1) == "E", 1,
                     ifelse(substr(bulkvar,nchar(toString(bulkvar))-1,nchar(toString(bulkvar))-1) ==2 | substr(bulkvar,nchar(toString(bulkvar))-1,nchar(toString(bulkvar))-1) == "D" | substr(bulkvar,nchar(toString(bulkvar))-1,nchar(toString(bulkvar))-1) == "F", 2, 
                            ifelse(substr(bulkvar,nchar(toString(bulkvar))-1,nchar(toString(bulkvar))-1) == 3, 3,
                                   ifelse(substr(bulkvar,nchar(toString(bulkvar))-1,nchar(toString(bulkvar))-1) == 4, 4,
                                          0)))))
        }
        
        # Baseband R503
        ProdInv$Qty[109] <- if(substr(bulkvar,nchar(toString(bulkvar))-1,nchar(toString(bulkvar))-1) == "*")     # that is no 5G
        {
          sum(ifelse(substr(bulkvar,23,23) == 8, 1, 0),
              ifelse(substr(bulkvar,48,48) == 1 | substr(bulkvar,48,48) == "C", 1, 
                     ifelse(substr(bulkvar,48,48) == 2 | substr(bulkvar,48,48) == "A" | substr(bulkvar,48,48) == "G", 2,
                            ifelse(substr(bulkvar,48,48) == 9, 4, 0))))
        }
        else
        {
          sum(ifelse(substr(bulkvar,23,23) == 8, 1, 0),
              ifelse(substr(bulkvar,48,48) == 1 | substr(bulkvar,48,48) == "C", 1, 
                     ifelse(substr(bulkvar,48,48) == 2 | substr(bulkvar,48,48) == "A" | substr(bulkvar,48,48) == "G", 2,
                            ifelse(substr(bulkvar,48,48) == 9, 4, 0))),
              ifelse(substr(bulkvar,nchar(toString(bulkvar))-2,nchar(toString(bulkvar))-2) == 1, 1, ifelse(substr(bulkvar,nchar(toString(bulkvar))-2,nchar(toString(bulkvar))-2) == 2, 2,
                                                                                                           ifelse(substr(bulkvar,nchar(toString(bulkvar))-2,nchar(toString(bulkvar))-2) == 3, 3, ifelse(substr(bulkvar,nchar(toString(bulkvar))-2,nchar(toString(bulkvar))-2) == 4, 4, 0)))))
        }
        ...
        ProdInv$Qty[113] <- sum(ifelse(substr(bulkvar,23,23) == 6, 1, 0), ifelse(substr(bulkvar,48,48) == "E", 1, 0), ifelse(substr(bulkvar,nchar(toString(bulkvar))-1,nchar(toString(bulkvar))-1) == 5, 1, 0))
        
        # Baseband 6502 (BB6502)
        ProdInv$Qty[114] <- ifelse(("Dark" %in% input$Config | substr(bulkvar,48,48) == "F" | "6502" %in% input$Config) & substr(bulkvar,46,47) == "sm", 1, 0)
        ...
        # Cabinets #
        # RBS6201
        ProdInv$Qty[124] <- ifelse((substr(bulkvar,8,9) == "id" | substr(bulkvar,20,21) == "id" | substr(bulkvar,46,47) == "id") & vSCD() == "B1-B3" & (DS6456[which(DS6456$TelefonicaConfiguration == input$Config),1] %in% cabsx2_B1B3), 2,
                                   ifelse((substr(bulkvar,8,9) == "id" | substr(bulkvar,20,21) == "id" | substr(bulkvar,46,47) == "id") & vSCD() == "B2-B3" & (DS6456[which(DS6456$TelefonicaConfiguration == input$Config),1] %in% cabsx2_B2B3), 2,
                                          ifelse((substr(bulkvar,8,9) == "id" | substr(bulkvar,20,21) == "id" | substr(bulkvar,46,47) == "id") & vSCD() == "New Build", 1,
                                                 ifelse((substr(bulkvar,8,9) == "id" | substr(bulkvar,20,21) == "id" | substr(bulkvar,46,47) == "id") & vSCD() == "B1-B3" & !(DS6456[which(DS6456$TelefonicaConfiguration == input$Config),1] %in% cabsx2_B1B3), 1,
                                                        ifelse((substr(bulkvar,8,9) == "id" | substr(bulkvar,20,21) == "id" | substr(bulkvar,46,47) == "id") & vSCD() == "B2-B3" & !(DS6456[which(DS6456$TelefonicaConfiguration == input$Config),1] %in% cabsx2_B2B3), 1,
                                                               0)))))
        
        # RBS6102
        ProdInv$Qty[125] <- ifelse((substr(bulkvar,8,9) == "oa" | substr(bulkvar,20,21) == "oa" | substr(bulkvar,20,21) == "oc" | substr(bulkvar,20,21) == "of" | substr(bulkvar,46,47) == "oa") &
        ...
        # York cabinet
        ProdInv$Qty[132] <- ifelse(substr(bulkvar,46,47) == "oy" | substr(bulkvar,46,47) == "of" | substr(bulkvar,46,47) == "ys",1,0)
        
        
        # Lancaster / Shire / 3rd party cabinet
        ProdInv$Qty[134] <- ifelse(substr(bulkvar,46,47) == "ys" & HLD() != HLDNB,1,0)
        
        ProdInv$Qty[135] <- ifelse(substr(bulkvar,46,47) == "ys" & HLD() == HLDNB,1,0)
        
        
        
        # RBS6202
        ProdInv$Qty[126] <- ifelse(substr(bulkvar,8,9) == "sc" | substr(bulkvar,20,21) == "oc" | substr(bulkvar,20,21) == "sc" | substr(bulkvar,46,47) == "sc", 1, 0)
        
        # RBS6147
        ProdInv$Qty[127] <- ifelse(substr(bulkvar,20,21) == "io" | substr(bulkvar,46,47) == "io", 1, 0)
        
        # RBS6601
        ProdInv$Qty[128] <- ifelse(substr(bulkvar,8,9) == "ir" | substr(bulkvar,20,21) == "ir" | substr(bulkvar,46,47) == "ir", 1, 0)     # removed counting RBS 6601 on substr(bulkvar,20,21) == "ib" |  substr(bulkvar,46,47) == "ib"
        
        # 19" Rack
        ProdInv$Qty[129] <- ifelse(substr(bulkvar,8,9) == "ir" | substr(bulkvar,20,21) == "ir" | substr(bulkvar,20,21) == "ib" | substr(bulkvar,46,47) == "ir" | substr(bulkvar,46,47) == "ib", 1, 0)
        
        # RBS6140/6150
        ProdInv$Qty[130] <- ifelse(substr(bulkvar,8,9) == "ob" | substr(bulkvar,20,21) == "ob" | substr(bulkvar,46,47) == "ob", 1, 0)
        
        # RBS6301
        ProdInv$Qty[131] <- ifelse(substr(bulkvar,46,47) == "or", 1, 0)
        
        # Eltek (Indoor/Outdoor)
        ProdInv$Qty[133] <- ifelse(substr(bulkvar,8,9) == "ie" | substr(bulkvar,20,21) == "ie" | substr(bulkvar,46,47) == "ie" |
                                     substr(bulkvar,8,9) == "oe" | substr(bulkvar,20,21) == "oe" | substr(bulkvar,46,47) == "oe", 1, 0)
        
        
        # reserved additional Cab/Enclosure dataframe positions, set to zero until required #
        # # RBS6320
        # ProdInv$Qty[134] <- ifelse(substr(bulkvar,8,9) == "od" | substr(bulkvar,20,21) == "od" | substr(bulkvar,46,47) == "od", 1, 0)
        # 
        # # RBS6501/RBS6502
        # ProdInv$Qty[135] <- ifelse("Dark" %in% input$Config | "6502" %in% input$Config, 0, ifelse(substr(bulkvar,20,21) == "sm" | substr(bulkvar,46,47) == "sm", 1, 0))
        ...
        bulkgen$IdleHDtotal[looper] <- sum(ProdInv$IdleHDtotal)
        
        bulkgen$TypPCtotal[looper] <- sum(ProdInv$TypPCtotal)
        bulkgen$TypOPtotal[looper] <- sum(ProdInv$TypOPtotal)
        bulkgen$TypHDtotal[looper] <- sum(ProdInv$TypHDtotal)
        
        bulkgen$HighPCtotal[looper] <- sum(ProdInv$HighPCtotal)
        bulkgen$HighOPtotal[looper] <- sum(ProdInv$HighOPtotal)
        bulkgen$HighHDtotal[looper] <- sum(ProdInv$HighHDtotal)
        
        bulkgen$MaxPCtotal[looper] <- sum(ProdInv$MaxPCtotal)
        bulkgen$MaxOPtotal[looper] <- sum(ProdInv$MaxOPtotal)
        bulkgen$MaxHDtotal[looper] <- sum(ProdInv$MaxHDtotal)
        
        # RUS 01 B20 (80W)
        bulkgen[looper,60] <-  ProdInv$Qty[39]
        # RUS 01 B1 (60/80W)
        bulkgen[looper,58] <-  ProdInv$Qty[17]
        # RUS 02 B1
        bulkgen[looper,61] <-  ProdInv$Qty[18] + ProdInv$Qty[35]
        # RUS 02 B8
        bulkgen[looper,63] <-  ProdInv$Qty[4] + ProdInv$Qty[5] + ProdInv$Qty[14] + ProdInv$Qty[15] + ProdInv$Qty[16] + ProdInv$Qty[56] + ProdInv$Qty[57] + ProdInv$Qty[58] + ProdInv$Qty[59] + ProdInv$Qty[60]
        # RUS 01 B3 (80W)
        bulkgen[looper,59] <-  ProdInv$Qty[36]
        # RUS 02 B3
        bulkgen[looper,62] <-  ProdInv$Qty[37]
        # RUS 03 B3
        bulkgen[looper,64] <-  ProdInv$Qty[38]
        # RRUS 11 B20 (2x40W)
        bulkgen[looper,65] <-  ProdInv$Qty[26]
        # RRUS 12 B8
        bulkgen[looper,66] <-  ProdInv$Qty[1]
        # Radio 2212 B8
        bulkgen[looper,67] <-  ProdInv$Qty[2] + ProdInv$Qty[3] + ProdInv$Qty[6] + ProdInv$Qty[29] + ProdInv$Qty[30] + ProdInv$Qty[55] + ProdInv$Qty[61]
        # Radio 2217 B1
        bulkgen[looper,68] <-  ProdInv$Qty[7] + ProdInv$Qty[8] + ProdInv$Qty[9] + ProdInv$Qty[10] + ProdInv$Qty[11] + ProdInv$Qty[12] + ProdInv$Qty[31] + ProdInv$Qty[52] + ProdInv$Qty[76] + ProdInv$Qty[77] + ProdInv$Qty[78] + ProdInv$Qty[79] + ProdInv$Qty[80] + ProdInv$Qty[81]
        # Radio 2217 B3
        bulkgen[looper,69] <-  ProdInv$Qty[32] + ProdInv$Qty[53]
        # Radio 2217 B8
        bulkgen[looper,70] <-  ProdInv$Qty[13] + ProdInv$Qty[33]
        # Radio 2217 B20
        bulkgen[looper,71] <-  ProdInv$Qty[34] + ProdInv$Qty[44] + ProdInv$Qty[45]
        # Radio 2219 B3
        bulkgen[looper,74] <-  ProdInv$Qty[19]
        # Radio 4415 B3
        bulkgen[looper,77] <-  ProdInv$Qty[20] + ProdInv$Qty[46] + ProdInv$Qty[47]
        # Radio 4415 B7A
        bulkgen[looper,78] <-  ProdInv$Qty[23]
        # Radio 8808 B40Y
        bulkgen[looper,81] <-  ProdInv$Qty[24] + ProdInv$Qty[25] + ProdInv$Qty[50] + ProdInv$Qty[51]
        # Radio 4415 B1
        bulkgen[looper,76] <-  ProdInv$Qty[21] + ProdInv$Qty[22] + ProdInv$Qty[48] + ProdInv$Qty[49]
        # Radio 8823 B42G
        ...
        # Radio 2203 B3
        bulkgen[looper,73] <-  ProdInv$Qty[54]
        # Radio 4480 B1 B3
        bulkgen[looper,79] <-  ProdInv$Qty[42] + ProdInv$Qty[43]
        # DUG 10
        bulkgen[looper,84] <-  ProdInv$Qty[99]
        # DUG 20
        bulkgen[looper,85] <-  ProdInv$Qty[100]
        # DUW 20
        bulkgen[looper,86] <-  ProdInv$Qty[101]
        # DUW 30
        bulkgen[looper,87] <-  ProdInv$Qty[102]
        # DUW 31
        bulkgen[looper,88] <-  ProdInv$Qty[103]
        # DUS 31
        bulkgen[looper,89] <-  ProdInv$Qty[104]
        # BB 5212
        bulkgen[looper,90] <-  ProdInv$Qty[88]
        # BB 5216
        bulkgen[looper,91] <-  ProdInv$Qty[89]
        # BB 6620
        bulkgen[looper,92] <-  ProdInv$Qty[107]
        # BB 6630
        bulkgen[looper,93] <-  ProdInv$Qty[108]
        # R503
        bulkgen[looper,94] <-  ProdInv$Qty[109]
        # BB 6318
        bulkgen[looper,98] <-  ProdInv$Qty[113]
        # BB 6502
        bulkgen[looper,99] <-  ProdInv$Qty[114]
        # RBS 6201
        bulkgen[looper,100] <-  ProdInv$Qty[124]
        # RBS 6202
        bulkgen[looper,101] <-  ProdInv$Qty[126]
        # Enc 6147/6215
        bulkgen[looper,102] <-  ProdInv$Qty[127]
        # RBS 6601
        bulkgen[looper,103] <-  ProdInv$Qty[128]
        # 19" Rack
        bulkgen[looper,104] <-  ProdInv$Qty[129]
        # Eltek
        bulkgen[looper,105] <-  ProdInv$Qty[133]
        # RBS 6102
        bulkgen[looper,106] <-  ProdInv$Qty[125]
        # RBS 6140/6150
        bulkgen[looper,107] <-  ProdInv$Qty[130]
        # RBS 6301
        bulkgen[looper,110] <-  ProdInv$Qty[131]
        # Lancaster 3rd Party Cab
        bulkgen[looper,111] <-  ProdInv$Qty[134]
        # Shire
        bulkgen[looper,113] <-  ProdInv$Qty[135]
        # York cabinet
        bulkgen[looper,112] <-  ProdInv$Qty[132]
...
v6C <- reactive({
  ...
  {
  # RUS 01 B20 (80W)
  RadioHW[3] <- v6B()$Qty[39]
  # RUS 01 B1 (60/80W)
  RadioHW[1] <- v6B()$Qty[17]
  # RUS 02 B1
  RadioHW[4] <- v6B()$Qty[18] + v6B()$Qty[35]
  # RUS 02 B8
  RadioHW[6] <- v6B()$Qty[4] + v6B()$Qty[5] + v6B()$Qty[14] + v6B()$Qty[15] + v6B()$Qty[16] + v6B()$Qty[56] + v6B()$Qty[57] + v6B()$Qty[58] + v6B()$Qty[59] + v6B()$Qty[60]
  # RUS 01 B3 (80W)
  RadioHW[2] <- v6B()$Qty[36]
  # RUS 02 B3
  RadioHW[5] <- v6B()$Qty[37]
  # RUS 03 B3
  RadioHW[7] <- v6B()$Qty[38]
  # RRUS 11 B20 (2x40W)
  RadioHW[8] <- v6B()$Qty[26]
  # RRUS 12 B8
  RadioHW[9] <- v6B()$Qty[1]
  # Radio 2212 B8
  RadioHW[10] <- v6B()$Qty[2] + v6B()$Qty[3] + v6B()$Qty[6] + v6B()$Qty[29] + v6B()$Qty[30] + v6B()$Qty[55] + v6B()$Qty[61]
  # Radio 2217 B1
  RadioHW[11] <- v6B()$Qty[7] + v6B()$Qty[8] + v6B()$Qty[9] + v6B()$Qty[10] + v6B()$Qty[11] + v6B()$Qty[12] + v6B()$Qty[31] + v6B()$Qty[52] + v6B()$Qty[76] + v6B()$Qty[77] + v6B()$Qty[78] + v6B()$Qty[79] + v6B()$Qty[80] + v6B()$Qty[81]
  # Radio 2217 B3
  RadioHW[12] <- v6B()$Qty[32] + v6B()$Qty[53]
  # Radio 2217 B8
  RadioHW[13] <- v6B()$Qty[13] + v6B()$Qty[33]
  # Radio 2217 B20
  RadioHW[14] <- v6B()$Qty[34] + v6B()$Qty[44] + v6B()$Qty[45]
  # Radio 2219 B3
  RadioHW[17] <- v6B()$Qty[19]
  # Radio 4415 B3
  RadioHW[20] <- v6B()$Qty[20] + v6B()$Qty[46] + v6B()$Qty[47]
  # Radio 4415 B7A
  RadioHW[21] <- v6B()$Qty[23]
  # Radio 8808 B40Y
  RadioHW[24] <- v6B()$Qty[24] + v6B()$Qty[25] + v6B()$Qty[50] + v6B()$Qty[51]
  # Radio 4415 B1
  RadioHW[19] <- v6B()$Qty[21] + v6B()$Qty[22] + v6B()$Qty[48] + v6B()$Qty[49]
  # Radio 8823 B42G
...
v6D <- reactive({  
  ...
  DUBBHW[1] <- v6B()$Qty[99]
  # DUG 20
  DUBBHW[2] <- v6B()$Qty[100]
  # DUW 20
  DUBBHW[3] <- v6B()$Qty[101]
  # DUW 30
  DUBBHW[4] <- v6B()$Qty[102]
  # DUW 31
  DUBBHW[5] <- v6B()$Qty[103]
  # DUS 31
  DUBBHW[6] <- v6B()$Qty[104]
  # BB 5212
  DUBBHW[7] <- v6B()$Qty[88]
  # BB 5216
  DUBBHW[8] <- v6B()$Qty[89]
  # BB 6620
  DUBBHW[9] <- v6B()$Qty[107]
  # BB 6630
  DUBBHW[10] <- v6B()$Qty[108]
  # R503
  DUBBHW[11] <- v6B()$Qty[109]
  # BB 6318
  DUBBHW[15] <- v6B()$Qty[113]
  # BB 6502
  DUBBHW[16] <- v6B()$Qty[114]
...
v6E <- reactive({
  ...
  CabHW[8] <- v6B()$Qty[130]
  # RBS 6301
  CabHW[11] <- v6B()$Qty[131]
  # Lancaster 3rd Party Cab
  CabHW[12] <- v6B()$Qty[134]
  # Shire
  CabHW[14] <- v6B()$Qty[135]
  # York cabinet
  CabHW[13] <- v6B()$Qty[132]
...
# rendering Bulk Generator Report #  
output$bulkgen <- DT::renderDataTable(
  v6B(), options = list(columnDefs = list(list(targets = c(6,7), visible = FALSE),list(className = 'dt-center', targets = c(1,4,8:69))),
                        scrollX = TRUE, pagelength = 100, lengthMenu = c(10,20,50,80,90,100), stringsAsFactors = FALSE)
)
...
## Generating Reports ##
  ...
  output$download <- downloadHandler(
    filename = function(){"PowerConsumption.csv"}, 
